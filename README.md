...
CorpBrain은 흩어져 있는 로컬 문서(`.txt`, `.md`, `.docx`)를 재귀적으로 탐색하고, 로컬 LLM(Ollama)을 활용하여 핵심 요약 및 태그가 포함된 마크다운 위키 페이지로 자동 구조화하는 도구입니다.
...
- **📁 로컬 문서 재귀 스캔**: 지정한 폴더 내의 `.docx`, `.txt`, `.md` 문서를 스캔하고 하위 디렉터리 구조를 그대로 미러링하여 생성.
...
│       ├── extract.py     # .txt, .md, .docx 텍스트 추출기
