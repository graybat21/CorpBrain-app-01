...
#: ISO 날짜(`YYYY-MM-DD`) 형태. 월·일 범위까지 확인해 진짜 날짜만 걸러낸다 —
#: `1234-56-78`처럼 날짜가 될 수 없는 값은 계좌번호 후보로 그대로 남긴다(누락 최소화).
_ISO_DATE = r"\d{4}-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])(?![\d-])"
...
PII_PATTERNS: tuple[tuple[PiiType, re.Pattern[str]], ...] = (
    ...
    (
        ...
        re.compile(
            ...
            # 오른쪽 경계에서 `.` 자체를 막으면 **문장 끝 이메일**(`문의는 a@b.co.kr.`)이
            # 통째로 빠져나간다. 도메인이 더 이어지는 경우(`.`+영숫자)만 거절한다.
    ...
    # ISO 날짜(`2026-08-21`)는 이 형태에 그대로 걸리는데, 계약서·회의록은 거의 모든 줄에
    # 날짜가 있어 요약이 "[REDACTED_ACCOUNT]에 시작해 [REDACTED_ACCOUNT]에 종료"처럼 뭉개지고
    # PII 집계도 노이즈로 부풀어 오른다. 날짜는 애초에 개인정보가 아니므로 제외한다.
    (
        PiiType.ACCOUNT,
        re.compile(_L + r"(?!" + _ISO_DATE + r")\d{2,6}-\d{2,6}-\d{2,6}(?:-\d{1,6})?" + _R),
    ),
    # IP주소 — IPv4 형태만. 옥텟 범위(0~255) 미검증. IPv6은 비목표.
    # 오른쪽 경계에서 `.`를 막으면 **문장 끝 IP**(`서버는 192.168.0.1.`)가 통째로 빠져나가므로
    # 막지 않는다. 대신 왼쪽 경계에서 `.`를 거절해 `1.2.3.4.5`의 중간(`2.3.4.5`)부터 매칭이
    # 시작되는 것을 막는다 — 앞에서부터 잡아 실제 IP를 남기지 않는 쪽을 택한다(누락 최소화).
    (PiiType.IP, re.compile(r"(?<![0-9A-Za-z_.])(?:\d{1,3}\.){3}\d{1,3}" + _R)),
)
