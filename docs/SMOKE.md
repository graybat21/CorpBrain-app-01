...
원인으로 보이는 것: 임베딩 입력이 원문이 아니라 **요약 텍스트**다(v0.4 §4.3). 모든 요약이
같은 템플릿과 한국어 사무 문서체를 공유해 **장르 유사도가 주제 유사도를 압도**한다. 탕비실
비품 요청 절차와 입사 절차는 둘 다 "사내 절차 안내"라 장르가 같고, 아키텍처 원칙과 벡터
스키마는 둘 다 기술 문서지만 어휘가 갈린다.

**기본값을 올리는 것으로는 해결되지 않는다.** 별도 판단이 필요하다(아래 «미결 사항»).
...
1. **유사도 엣지를 어떻게 할 것인가** (발견 1). 기본값 상향으로는 해결되지 않는다.
   검토할 만한 방향: 절대 임계치 대신 문서당 상대 순위(top-N) · 임베딩 입력을 요약이 아닌
   원문으로 · 유사도 엣지를 옵트인으로 · 현행 유지하고 알려진 한계로 문서화.
