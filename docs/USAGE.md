...
| 기능 | v0.5.x | v0.6.x | v0.7.0 | v0.8.0 |
|---|---|---|---|---|
| 위 v0.4까지의 기능 | 동일 | 동일 | 동일 | 동일 |
| 클라우드 옵트인(`--engine cloud`·동의·PII 마스킹) | ✅ | 동일 | 동일 | 동일 |
| **지식그래프**(scan에 통합) | — | ✅ | 동일 | 동일 |
| 위키 「관련 문서」 섹션 | — | ✅ | 동일 (한 글자도 바뀌지 않는다) | 동일 |
| `corpbrain graph` — 그래프 조회 | — | ✅ | 동일 | 동일 |
| **`search` 그래프 시드 확산**(기본 켜짐, `--no-graph`로 끔) | — | — | ✅ (기본 α에서는 순위 불변 — §6.3) | 동일 |
| 지원 입력 포맷 | 동일 | 동일 | 동일 | + **`.xlsx` `.xlsm` `.pptx`** |
