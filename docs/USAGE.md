...
| **`search` 그래프 시드 확산**(기본 켜짐, `--no-graph`로 끔) | — | — | ✅ |
...
| `--graph-decay FLOAT` `[v0.7]` | `0.7` (잠정) | 확산 감쇠 계수 α — **0과 1 사이**(양 끝 제외) |
...
> ⚠️ `--graph-decay` 기본값 `0.7`은 **실측 전 잠정값**이다(스윕 범위 0.5~0.95의 가운데).
> `docs/smoke/graph_decay_sweep.py`로 α를 스윕해 확정할 예정이며, 확정 전까지는 값의 근거가
> 없다는 뜻이다.
