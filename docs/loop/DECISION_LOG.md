...
## 사용자 확정 (루프 종료 후, 재개 시 적용)
- [사용자결정] U-001 | FR-012 | 2026-08-12 | 출력 파일명은 원본 파일명을 유지한 `<원본이름.확장자>.md` (예: `report.docx` → `report.docx.md`, `a.txt` → `a.txt.md`) | 근거: 이슈 21행("원본 확장자를 .md로 대체하지 말라")을 따르고, 같은 폴더의 `a.txt`/`a.md`가 같은 출력으로 충돌해 "1파일 1위키"가 깨지는 것을 원천 차단. 이슈 FR-012 Scenario 1의 기대 트리(`sub/a.md`) 문구는 재개 시 이 규칙에 맞춰 수정 필요.
- [사용자결정] U-002 | 루프 | 2026-08-12 | MINOR_BUDGET 도달 시점에서 루프를 종료하고 8/20 이슈 상태를 유지한다(예산 리셋·상향 없음). 재개는 별도 /goal로 한다.
- [사용자결정] U-003 | 전체 | 2026-08-12 | 위 MINOR 10건을 승인하고 `docs/decisions/implementation-decisions.md`로 정본화. 제품 동작 계약(출력 파일명 규칙)은 스펙 §3-1·§4.4와 `docs/issues/FR-012.md`에 직접 반영 | 근거: 정본 문서에 반영된 항목은 이후 루프에서 "미결정"으로 다시 카운트하지 않기 위함.
