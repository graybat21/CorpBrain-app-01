...
- [CORE] PII 정규식 7종의 경계를 스펙 §4.5 표의 `\b` 대신 **ASCII 전용 lookaround**
  ...
  - **스펙 후속 조치 — 2026-08-21 해소됨**: 루프 종료 후 사용자 지시로
    `static/docs/specs/features/corpbrain-v0.5-cloud-opt-in.md`를 구현과 일치시켰다
    (§4.5 경계 규칙·패턴 표·ISO 날짜 제외, §4.3 타임아웃·프리플라이트 분류, §2 임베딩 문구,
    신규 §4.8, 완료의 정의 14~16 추가). 루프 진행 중에는 스펙이 수정 금지 대상이었다.
