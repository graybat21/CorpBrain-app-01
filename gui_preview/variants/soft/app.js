...
function initGraphCanvas() {
  ...
  function render() {
    ...
    edges.forEach(e => {
      ...
      if (e.type === "ref") {
        ctx.strokeStyle = isConnected ? "#5cc9dd" : "rgba(92, 201, 221, 0.28)";
        ctx.lineWidth = isConnected ? 2.5 : 1.2;
        ctx.setLineDash([]);
      } else if (e.type === "entity") {
        ctx.strokeStyle = isConnected ? "#e2a23d" : "rgba(226, 162, 61, 0.22)";
        ctx.lineWidth = isConnected ? 2 : 1;
        ctx.setLineDash([]);
      } else {
        ctx.strokeStyle = isConnected ? "#9b87f5" : "rgba(155, 135, 245, 0.2)";
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 4]);
      }
      ctx.stroke();
    ...
    nodes.forEach(n => {
      ...
      if (n.type === "doc") {
        fill = "#5cc9dd";
        ctx.shadowColor = "#5cc9dd";
      } else if (n.type === "entity") {
        fill = "#e2a23d";
        ctx.shadowColor = "#e2a23d";
      } else {
        fill = "#9b87f5";
        ctx.shadowColor = "#9b87f5";
      }

      ctx.shadowBlur = (isHovered || isSelected) ? 18 : 7;
      ctx.fillStyle = fill;
      ctx.fill();
      ctx.shadowBlur = 0;

      ctx.lineWidth = isSelected ? 3 : 1.5;
      ctx.strokeStyle = isSelected ? "#f3f3f6" : "rgba(255, 255, 255, 0.35)";
      ctx.stroke();

      ctx.font = `${isHovered ? "600 12px" : "500 11px"} 'Space Grotesk', 'Pretendard', sans-serif`;
      ctx.fillStyle = isHovered ? "#f3f3f6" : "#9d9da8";
