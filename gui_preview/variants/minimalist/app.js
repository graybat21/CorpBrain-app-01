...
// --- Inline Icon Library (line icons, no emoji) ---
const ICON_SVGS = {
  check: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="M4.5 12.5 9 17l10.5-10.5"/></svg>',
  lock: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="10.5" width="14" height="9.5" rx="1.5"/><path d="M8 10.5V7.5a4 4 0 0 1 8 0v3"/></svg>',
  ruler: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="8" width="18" height="8" rx="1.2"/><path d="M7 8v3M11 8v2M15 8v3M19 8v2"/></svg>',
  package: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3 4 7v10l8 4 8-4V7z"/><path d="M4 7l8 4 8-4M12 11v10"/></svg>',
  users: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3"/><path d="M3.5 19a5.5 5.5 0 0 1 11 0"/><circle cx="17" cy="9" r="2.3"/><path d="M15.3 19a4.2 4.2 0 0 1 5-4"/></svg>',
  scale: '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v18M7 21h10"/><path d="M5 7h6M13 7h6"/><path d="M5 7 2.5 12.5a2.6 2.6 0 0 0 5 0zM19 7l-2.5 5.5a2.6 2.6 0 0 0 5 0z"/></svg>',
  folder: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6.5c0-.8.7-1.5 1.5-1.5H9l2 2.2h8.5c.8 0 1.5.7 1.5 1.5v9.3c0 .8-.7 1.5-1.5 1.5h-15C3.7 19.5 3 18.8 3 18V6.5z"/></svg>',
  file: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3.5h8l4 4v13a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1v-16a1 1 0 0 1 1-1z"/><path d="M14 3.5V8h4"/></svg>',
  pin: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-6.5-6.1-6.5-11A6.5 6.5 0 0 1 18.5 10c0 4.9-6.5 11-6.5 11z"/><circle cx="12" cy="10" r="2.2"/></svg>',
  list: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="4.5" width="14" height="16" rx="1.5"/><path d="M9 4.5V3.5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v1M8 10h8M8 13.5h8M8 17h5"/></svg>',
  tag: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4h6.5A2.5 2.5 0 0 1 20 6.5V13a1.5 1.5 0 0 1-.44 1.06l-8 8a1.5 1.5 0 0 1-2.12 0l-6.5-6.5a1.5 1.5 0 0 1 0-2.12l8-8A1.5 1.5 0 0 1 11 4z"/><circle cx="15.5" cy="8.5" r="1.2" fill="currentColor" stroke="none"/></svg>',
  network: '<svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="1.8"/><circle cx="18" cy="6" r="1.8"/><circle cx="6" cy="18" r="1.8"/><circle cx="18" cy="18" r="1.8"/><circle cx="12" cy="12" r="1.8"/><path d="M7.4 7.4 10.6 10.6M16.6 7.4 13.4 10.6M7.4 16.6 10.6 13.4M16.6 16.6 13.4 13.4"/></svg>'
};

function icon(name) {
  return `<span class="icon">${ICON_SVGS[name] || ""}</span>`;
}

// --- Keyboard activation for custom interactive elements (non-<button>/<a> nav items) ---
function makeActivatable(el) {
  if (!el || el.dataset.activatable) return;
  el.dataset.activatable = "1";
...
// --- In-system modal/toast (replaces native window.prompt/window.alert) ---
function showModal({ title, desc, defaultValue = "" }) {
  const overlay = document.getElementById("app-modal-overlay");
  const titleEl = document.getElementById("app-modal-title");
  const descEl = overlay.querySelector(".app-modal-desc");
...
function showToast(message, duration = 3500) {
  ...
  appToastTimer = setTimeout(() => { toast.hidden = true; }, duration);
}
...
const MULTI_PROJECTS = {
  p1: {
    ...
    activities: [
      { icon: "check", text: "연구개발팀 아키텍처 문서 스캔 및 지식그래프 갱신", time: "방금 전 · 3개 파일 처리 완료" },
      { icon: "lock", text: "사내 보안 가이드라인 PII 마스킹 (5건 보호 처리)", time: "15분 전 · security/vpn_access_guide.pdf" },
      { icon: "ruler", text: "인사 규정 문서 벡터 임베딩 인덱스 동기화", time: "1시간 전 · .corpbrain_index.sqlite" }
    ],
    wikiTree: [
      {
  ...
  p2: {
    ...
    activities: [
      { icon: "package", text: "v0.4 릴리스 베이스라인 인덱스 동기화 완료", time: "어제 · 18개 문서 인덱싱" }
    ],
    wikiTree: [
      {
        folder: "01. 코어 문서",
  ...
  p3: {
    ...
    activities: [
      { icon: "users", text: "2026년 복무 규정 및 평가 지침 통합 스캔", time: "3일 전 · 12개 파일 완료" }
    ],
    wikiTree: [
      {
        folder: "인사 총무 규정",
  ...
  p4: {
    ...
    activities: [
      { icon: "scale", text: "표준 계약서 양식 및 개인정보 처리방침 스캔", time: "5일 전 · 24개 파일 완료" }
    ],
    wikiTree: [
      {
        folder: "법무 및 계약",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Static connected-node pills in the wiki context panel (inline onclick markup)
  document.querySelectorAll(".wiki-context-panel .connected-node-item").forEach(makeActivatable);
});
...
function initProjectNavigation() {
  ...
  const handleAdd = async () => {
    const folderName = await showModal({
      title: "새 로컬 프로젝트 폴더 열기",
      desc: "추가할 로컬 프로젝트 폴더명을 입력하세요.",
      defaultValue: "New-Project-2026"
    });
    if (folderName) {
      showToast(`'${folderName}' 폴더가 워크스페이스에 등록되었습니다. 상단 '플랜 & 스캔' 탭에서 인덱싱을 진행해 주세요.`);
    }
  };
...
function switchProject(pid) {
  ...
  // Close the graph inspector so a stale node from the previous project's
  // graph is never left on screen after switching projects.
  AppState.selectedNode = null;
  const inspectorPanel = document.getElementById("graph-inspector-panel");
...
function initScanHub() {
  ...
  const skipSummary = document.getElementById("scan-skip-summary");
  const skipCount = document.getElementById("scan-skip-count");
  const skipList = document.getElementById("scan-skip-list");
  ...
  btnCancelScan.addEventListener("click", () => {
    ...
    addLog("취소됨", "사용자 요청으로 스캔이 중단되었습니다. 이미 처리된 파일의 위키는 유지됩니다.", "stage-skip");
  });
  ...
  btnScan.addEventListener("click", () => {
    ...
    const files = [
      ...
      { name: "misc/legacy_notes.hwp", size: "420KB", skip: "미지원 확장자 (.hwp)" }
    ];
    ...
    scanInterval = setInterval(() => {
      if (fileIdx >= files.length) {
        ...
        addLog("완료", `총 ${files.length}개 파일 처리: ${processedCount}개 위키 생성, ${skipped.length}개 스킵, 벡터 인덱싱 및 지식그래프 갱신 완료`, "stage-done");
        if (skipped.length > 0) {
          skipSummary.hidden = false;
          skipCount.textContent = `${skipped.length}개`;
          skipList.innerHTML = skipped.map(s => `<li><strong>${s.name}</strong> — ${s.reason}</li>`).join("");
        }
        return;
      }
      ...
      progressFill.style.transform = `scaleX(${AppState.scanProgress / 100})`;
      progressPercent.textContent = `${AppState.scanProgress}%`;

      if (f.skip) {
        skipped.push({ name: f.name, reason: f.skip });
        addLog(f.name, `스킵: ${f.skip}`, "stage-skip");
      } else {
        addLog(f.name, "텍스트 추출 완료 (" + f.size + ")", "stage-extract");
        setTimeout(() => { if (AppState.isScanning) addLog(f.name, "Ollama 요약 완료 (예상 토큰 " + f.est + "t)", "stage-summarize"); }, 200);
        setTimeout(() => { if (AppState.isScanning) addLog(f.name, "PII 검사 및 마스킹 적용 완료", "stage-pii"); }, 400);
        setTimeout(() => { if (AppState.isScanning) addLog(f.name, "벡터 임베딩 및 지식그래프 노드/엣지 연결 완료", "stage-embed"); }, 600);
      }
...
function initGraphCanvas() {
  ...
  function loadDataset(graphData) {
    ...
    if (emptyState) emptyState.hidden = nodes.length > 0;
  }
  ...
  function render() {
    ...
    edges.forEach(e => {
      ...
      if (e.type === "ref") {
        ctx.strokeStyle = isConnected ? "#1F6C9F" : "rgba(31, 108, 159, 0.3)";
        ctx.lineWidth = isConnected ? 2.5 : 1.2;
        ctx.setLineDash([]);
      } else if (e.type === "entity") {
        ctx.strokeStyle = isConnected ? "#956400" : "rgba(149, 100, 0, 0.25)";
        ctx.lineWidth = isConnected ? 2 : 1;
        ctx.setLineDash([]);
      } else {
        ctx.strokeStyle = isConnected ? "#5B4B8A" : "rgba(91, 75, 138, 0.22)";
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 4]);
      }
      ctx.stroke();
    ...
    nodes.forEach(n => {
      ...
      ctx.shadowBlur = (isHovered || isSelected) ? 8 : 0;
      ctx.fillStyle = fill;
      ctx.fill();
      ctx.shadowBlur = 0;

      ctx.lineWidth = isSelected ? 3 : 1.5;
      ctx.strokeStyle = isSelected ? "#111111" : "rgba(255, 255, 255, 0.85)";
      ctx.stroke();

      ctx.font = `${isHovered ? "bold 12px" : "11px"} Pretendard, -apple-system, sans-serif`;
      ctx.fillStyle = isHovered ? "#111111" : "#56534C";
...
function initWikiReader() {
  ...
  tree.forEach(folderGroup => {
    ...
    groupEl.innerHTML = `<div class="tree-folder-title">${icon("folder")} ${folderGroup.folder}</div>`;
    ...
    folderGroup.docs.forEach(d => {
      ...
      li.innerHTML = `${icon("file")} <span>${d.title}</span>`;
      li.onclick = () => {
...
function renderWikiDetail(wikiId) {
  ...
  mainEl.innerHTML = `
    ...
    <div class="wiki-document-body">
      ...
      <h2>${icon("file")} 상세 요약 본문</h2>
      <p>${doc.body.replace(/\n\n/g, "</p><p>").replace(/\[PII:[^\]]+\]/g, m => `<span class="pii-highlight">${m}</span>`)}</p>

      <h2>${icon("tag")} 연관 태그 & 엔티티</h2>
      <div class="tag-list" style="margin-top:10px;">
        ${doc.tags.map(t => `<span class="tag-badge">#${t}</span>`).join("")}
        ${doc.entities.map(e => `<span class="tag-badge" style="background:var(--pastel-yellow-bg); color:var(--accent-amber)">@${e}</span>`).join("")}
      </div>
    </div>
  `;
}
    ...
    mockSearchResults.forEach(res => {
      ...
      card.innerHTML = `
        ...
        <div class="result-footer">
          ...
          <button class="btn btn-outline btn-sm">${icon("network")} 지식그래프에서 보기</button>
        </div>
      `;
