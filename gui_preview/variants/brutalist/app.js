...
function initGraphCanvas() {
  ...
  function render() {
    ...
    edges.forEach(e => {
      ...
      if (e.type === "ref") {
        ctx.strokeStyle = isConnected ? "#ff2a2a" : "#3a1414";
        ctx.lineWidth = isConnected ? 2.5 : 1.2;
        ctx.setLineDash([]);
      } else if (e.type === "entity") {
        ctx.strokeStyle = isConnected ? "#c98a2c" : "#3a2c14";
        ctx.lineWidth = isConnected ? 2 : 1;
        ctx.setLineDash([]);
      } else {
        ctx.strokeStyle = isConnected ? "#8a7086" : "#2c2630";
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 4]);
      }
      ctx.stroke();
    ...
    nodes.forEach(n => {
      ...
      if (n.type === "doc") {
        ...
        fill = "#c98a2c";
        ctx.shadowColor = "#c98a2c";
      } else {
        fill = "#8a7086";
        ctx.shadowColor = "#8a7086";
      }
      ...
      ctx.strokeStyle = isSelected ? "#eaeaea" : "#3a3a3a";
      ctx.stroke();

      ctx.font = `${isHovered ? "bold 11px" : "10px"} 'JetBrains Mono', 'Pretendard', sans-serif`;
      ctx.fillStyle = isHovered ? "#eaeaea" : "#8a8a8a";
