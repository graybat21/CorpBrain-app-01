...
13. `<out_dir>`에 이미 다른 임베딩 모델로 생성된 인덱스가 있으면(기록된 `model_name` ≠ `--embed-model`), `scan`은 파일을 처리하지 않고 **exit 1**로 종료하며 모델을 맞추거나 인덱스를 지우고 `--force`로 다시 생성하라는 안내를 낸다. `--force-gates`로 우회되지 않는다(요약·임베딩 모델 프리플라이트와 동급 선행 조건 — 서로 다른 모델의 벡터가 한 인덱스에 섞이면 코사인 유사도 비교 자체가 무의미해지기 때문이다). 인덱스가 비어 있으면(신규) 제약 없이 그대로 기록한다.
    - 검증: 통합테스트 — 서로 다른 `--embed-model`로 같은 `--out`에 연속 `scan`, 두 번째 실행에서 exit 1·처리 0건·안내 문자열 확인. [사용자 결정 · 2026-08-19 추가 — `/code-review` 발견 반영]
...
flowchart TD
    ...
    F2 -- 미지정 --> F1["exit 1\n--force-gates 안내"]
    F2 -- 지정 --> K{"⑦ 인덱스 임베딩 모델 일치\n(기존 model_name == --embed-model?)"}
    F -- 통과 --> G{"⑥ 토큰 게이트\ntotal_est_tokens ≤ max?"}
    G -- 초과 --> G2{"--force-gates ?"}
    G2 -- 미지정 --> G1["exit 3\n토큰 상한 초과"]
    G2 -- 지정 --> K
    G -- 통과 --> K
    K -- 불일치 --> K1["exit 1\n모델을 맞추거나 --force로 재생성 안내"]
    K -- 일치(또는 신규 인덱스) --> H["파일별 처리\n추출 → 요약 → 렌더 → 임베딩 → 인덱스 기록"]
    H --> I["종료 리포트\nexit 0 (부분 성공 가능)"]

    style E fill:#fff3cd,stroke:#333
    style E1 fill:#fff3cd,stroke:#333
    style K fill:#e0f0ff,stroke:#333
    style K1 fill:#e0f0ff,stroke:#333
```

①~④(폴더·Ollama·요약모델·임베딩모델)와 ⑦(인덱스 임베딩 모델 일치)은 `--force-gates`로 우회 불가. ⑤⑥(GPU·토큰)만 우회 가능. [사용자 결정 · ⑦은 2026-08-19 추가]
...
- **임베딩 모델 일치 강제**: `scan`은 인덱스를 열 때 기존 메타데이터의 임베딩 모델명과 `config.embed_model`이 다르면 즉시 거부한다(exit 1, `--force-gates` 무관, §3 항목13) — `search` 쪽의 "인덱스 기록 모델을 강제 사용"(§3 항목7)과 대칭을 이루는 `scan` 쪽 불변식이다. 인덱스가 비어 있으면(신규) 제약 없이 그대로 기록한다. [사용자 결정 · 2026-08-19 추가 — `/code-review` 발견 반영]
- **어댑터 인터페이스 계약**: `upsert(doc_id: str, vector: list[float], metadata: dict) -> None`, `delete(doc_id: str) -> None`, `search(query_vector: list[float], top_k: int) -> list[SearchResult]`, `list_ids() -> list[str]`(저장된 모든 `doc_id` — 고아 벡터 정리에 쓴다, §3 항목5), `set_model_name(model_name: str) -> None`(인덱스가 기록할 임베딩 모델명을 멱등하게 설정), `close() -> None`(저장소가 쥔 자원을 정리) — 6메서드 + `model_name: str | None`(인덱스가 기록한 임베딩 모델명 접근자, 아직 없으면 `None`) 속성 하나로 구성한다. 파이프라인·`search` CLI는 이 인터페이스 타입에만 의존하고 구체 구현(`sqlite3` 기반)을 직접 참조하지 않는다. [사용자 결정 · 2026-08-19 갱신 — 최초 3메서드 계약에서, 파이프라인·`search`가 실제로 호출하는 `list_ids`/`set_model_name`/`close`를 반영해 확장(`/code-review` 발견 반영)]
...
- 이미 다른 임베딩 모델로 생성된 인덱스에 다른 `--embed-model`로 재스캔: `scan`이 exit 1 + 모델을 맞추거나 `--force`로 인덱스를 재생성하라는 안내를 낸다(§3 항목13).
